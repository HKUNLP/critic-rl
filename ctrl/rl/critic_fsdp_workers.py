...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        if (
            self._is_actor and optim_config is not None
        ):  # NOTE: we use actor_rollout_ref
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == "hf":
            ...
            from ctrl.rl.critic_vllm_rollout import vLLMRollout
            ...
            # NOTE: apply monkey patch to FSDPVLLMShardingManager
            ...
            # Define update_module as a separate method
            def _create_updated_sharding_manager():
                class UpdatedFSDPVLLMShardingManager(FSDPVLLMShardingManager):
                    def update_module(self, module):
                        self.module = module
                        state_dict_config = (
                            FullStateDictConfig()
                            if self.full_params
                            else ShardedStateDictConfig()
                        )
                        state_dict_type = (
                ...
                return UpdatedFSDPVLLMShardingManager

            ShardingManagerClass = _create_updated_sharding_manager()
            rollout_sharding_manager = ShardingManagerClass(
    ...
    def init_model(self):
        ...
        if self.config.model.path != self.config.proxy.model.path:
            self.proxy_module_fsdp = self._build_model_optimizer(
                model_path=self.config.proxy.model.path,
                fsdp_config=self.config.ref.fsdp_config,
                optim_config=None,
                override_model_config=override_model_config,
                trust_remote_code=self.config.proxy.model.get(
                    "trust_remote_code", False
                ),
            )[0]
            if self._is_offload_param:
                offload_fsdp_param_and_grad(
                    module=self.proxy_module_fsdp, offload_grad=True
                )
                log_gpu_memory_usage(
                    "After offload proxy grad during init", logger=logger
                )
            self.is_same_proxy = False
        else:
            self.proxy_module_fsdp = self.ref_module_fsdp
            self.is_same_proxy = True
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_sequences(self, prompts: DataProto, use_proxy=False, **kwargs):
        prompts = prompts.to("cuda")

        assert self._is_rollout
        module_fsdp = self.proxy_module_fsdp if use_proxy else self.actor_module_fsdp
        if self._is_offload_param and (not use_proxy or not self.is_same_proxy):
            load_fsdp_param_and_grad(
                module=module_fsdp,
                device_id=torch.cuda.current_device(),
                load_grad=self._is_offload_grad,
            )
        self.rollout_sharding_manager.update_module(module_fsdp)
        ...
        if self._is_offload_param:
            ...
            if use_proxy:
                offload_fsdp_param_and_grad(
                    module=self.proxy_module_fsdp, offload_grad=self._is_offload_grad
                )
            else:
                offload_fsdp_param_and_grad(
                    module=self.actor_module_fsdp, offload_grad=self._is_offload_grad
    ...
    def generate_sequences_ref(self, prompts: DataProto):
        config_max_tokens = self.rollout.config.response_length
        new_max_tokens = 1024
        self.rollout.config.response_length = new_max_tokens
        output = self.generate_sequences(
            prompts=prompts, use_proxy=True, max_tokens=new_max_tokens
        )
        self.rollout.config.response_length = config_max_tokens
        return output
